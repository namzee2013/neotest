...
<div class="page-content-wrap">
    <div class="col-md-12">
        <form class="form-horizontal" action="{{ asset('admin/question/edit-check-quest') }}/{{ $question['id'] }}" method="POST">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <div class="panel panel-default">
                <!-- START DEFAULT FORM ELEMENTS -->
                <div class="panel-body">
                    <h4>Edit Multi-choice Question</h4>
                    ...
                    @foreach ($option as $item)
                        <div class="form-group option">
                            ...
                            @if ($item['status'] == 1)
                                <input type="checkbox" value="{{ $item['id']}}" name="correct[{{ $item['id']}}]" checked>
                            @else
                                <input type="checkbox" value="{{ $item['id']}}"  name="correct[{{ $item['id']}}]">
                            @endif
                             Option</label>
                            <div class="col-md-7">
...
<script type="text/javascript">
    ...
    $(".btn-add-more-option").on("click", function(e) {
        tempopt = '<div class="form-group option">'
            +'<label class="col-md-3 control-label"><input type="checkbox" value="'+i+'" id="check'+i+'" name="newcorrect['+i+']"> Option</label>'
            +'<div class="col-md-7">'
