      ...
      <form class="form-test col-sm-6">
        ...
        @foreach ($array as $key => $question_option)
            @foreach ($question_option['question'] as $key => $value) 
                ...
                @if($value['image'])
                    <img id="images-question" src="{{ asset('uploads/images') }}/{{$value['image']}}" alt="">
                @endif
                @if($value['type_id'] == 1)
                    @foreach ($question_option['option'] as $key => $value)
                        <div class="option">                
                            <input id="{{$value['id']}}" option_id="{{$value['id']}}" type="radio" name="{{{$question_id}}}">
                            <p>
                                {{$value['content']}}
                            </p>
                            <br>
                ...
                @elseif($value['type_id'] == 2)
                    @foreach ($question_option['option'] as $key => $value)
                        <div class="option">
                            <input id="{{$value['id']}}" option_id="{{$value['id']}}" type="checkbox" name="{{$question_id}}">
                            <p>
                                {{$value['content']}}
                            </p>
                            <br>
                ...
                @elseif($value['type_id'] == 3)
                    <div class="col-sm-12">
                        <textarea id="{{$question_id}}" class="textarea form-control col-sm-3" placeholder="Answer the question" value="" rows="5"></textarea>
                    </div>
                @endif
            @endforeach
        @endforeach
        <br>
        <div class="btn">
            <a href="{{route('eachtest',[$link,1])}}" class="btn-each">Each Question</a>
            <a id="submit" link="{{$link}}" email="{{$email}}" class="btn-submit submit">Submit</a>            
        </div>
    </form>
    </div>
  </div>
